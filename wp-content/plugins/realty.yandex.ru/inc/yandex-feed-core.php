...
class La_Yandex_Feed_Core {
	...
	public function custom_query_vars(){
		...
		//custom
		$slug = trailingslashit(get_option('layf_custom_url', 'yandex/feed/')); //var_dump($slug);
		
		if(!empty($slug) && $slug != '/' && $slug != 'yandex/feed/'){
			add_rewrite_rule("^$slug?", 'index.php?yandex_feed=feed', 'top');
		}
	...
	public function custom_request($query) {
		if (isset($query->query_vars['yandex_feed']) && $query->query_vars['yandex_feed'] == 'feed') {	
			$query->query_vars['post_type'] = 'object';
	...
	public function custom_templates_redirect(){
		...
		if (get_query_var('yandex_feed') == "feed"){
            #if (!$this->isCache('/var/www/vipest.ru/public/xml/yandex_feed.xml')) {
                #ob_start();
                
                require_once LAYF_PLUGIN_DIR . 'inc/feed.php';
                
                $out = ob_get_clean();
                $out = preg_replace('/(?![^<]*<\/pre>)[\n\r\t]+/', "\n", $out);
                $out = preg_replace('/ {2,}/', ' ', $out);

                #$f = fopen('/var/www/vipest.ru/public/xml/yandex_feed.xml', 'w');
                #fwrite($f, $out);
                #fclose($f);
